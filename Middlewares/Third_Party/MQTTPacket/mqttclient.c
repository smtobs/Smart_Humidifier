...
uint8_t MQTT_Connect(void)
{
	...
	//transporter.sck = 0; //&transport_socket;
	...
	//ret = Wifi_TcpIp_SendDataUdp(0, (uint16_t)len, buf);
	 //ret =  Wifi_TcpIp_SendDataTcp(0, (uint16_t)len, buf);
	ret = Wifi_TcpIp_StartMqttConnection(0, (uint16_t)len, buf);

	 if (ret == true)
	 {
		 HAL_UART_Transmit(&huart1, (uint8_t *)"ret true \r\n", strlen("ret true \r\n"), 1000);
	 }
	 else
	 {
		 HAL_UART_Transmit(&huart1, (uint8_t *)"ret false \r\n", strlen("ret false \r\n"), 1000);
	 }
	...
	if (MQTTPacket_read(buf, buflen, transport_getdata) == CONNACK)
	{
		...
		if (MQTTDeserialize_connack(&sessionPresent, &connack_rc, buf, buflen) != 1 || connack_rc != 0)
		{
			...
			HAL_UART_Transmit(&huart1, (uint8_t *)"Connect_NOK, the rc is\r\n", (uint16_t)strlen("Connect_NOK, the rc is\r\n"), 1000);
			return Connect_NOK;
		}
		else
		{
...
void Client_Connect(void)
{
	...
	if (MQTT_Connect() != Connect_OK)
	{
		...
		HAL_UART_Transmit(&huart1, (uint8_t *)"MQTT Connect Fail\r\n", (uint16_t)strlen("MQTT Connect Fail\r\n"), 1000);
		goto MQTT_START;
	}
	else
	{
		HAL_UART_Transmit(&huart1, (uint8_t *)"MQTT Connect Success\r\n", (uint16_t)strlen("MQTT Connect Success\r\n"), 1000);
	}
...
int32_t MQTTMsgPublish(int32_t sock, char *topic, int8_t qos, uint8_t* msg)
{
	...
	//if (transport_sendPacketBuffer(buf, len) < 0)
	ret = Wifi_TcpIp_SendDataTcp(0, (uint16_t)len, (uint8_t *)buf);
	//ret = Wifi_TcpIp_SendDataUdp(0, (uint16_t)len, buf);
	 if (ret == true)
	 {
		 HAL_UART_Transmit(&huart1, (uint8_t *)"ret true \r\n", strlen("ret true \r\n"), 1000);
